...
class Flux2DoubleXEmbedderTransformer2DModel(
    ModelMixin,
    ConfigMixin,
    PeftAdapterMixin,
    FromOriginalModelMixin,
    ...
    def __init__(
        ...
        # 4. Input projections
        self.x_embedder = nn.Linear(in_channels, self.inner_dim, bias=False)
        self.cond_x_embedder = nn.Linear(cond_in_channels, self.inner_dim, bias=False)
        self.context_embedder = nn.Linear(
            joint_attention_dim, self.inner_dim, bias=False
        )
    ...
    def forward(
        ...
        Args:
            hidden_states (`torch.Tensor` of shape `(batch_size, image_sequence_length, in_channels)`):
                Input `hidden_states`. This is the noisy latents.
            cond_hidden_states (`torch.Tensor` of shape `(batch_size, cond_sequence_length, cond_in_channels)`):
                Input `cond_hidden_states`. This is the embeddings computed from the conditioning images.
            encoder_hidden_states (`torch.Tensor` of shape `(batch_size, text_sequence_length, joint_attention_dim)`):
                Conditional embeddings (embeddings computed from the input conditions such as prompts) to use.
            timestep (`torch.LongTensor`):
                Used to indicate denoising step.
        ...
        # 2. Input projection for image (hidden_states) and conditioning text (encoder_hidden_states)
        hidden_states = self.x_embedder(hidden_states)
        cond_hidden_states = self.cond_x_embedder(cond_hidden_states)
        encoder_hidden_states = self.context_embedder(encoder_hidden_states)

        # Concat the hidden_states and cond_hidden_states
