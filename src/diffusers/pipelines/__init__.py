...
try:
    ...
    _import_structure["hunyuan_image"] = [
    ...
    _import_structure["longcat_image"] = [
...
try:
    ...
    _dummy_objects.update(
        get_objects_from_module(dummy_torch_and_transformers_and_sentencepiece_objects)
    )
else:
...
try:
    if not (
        is_torch_available() and is_transformers_available() and is_opencv_available()
    ):
        raise OptionalDependencyNotAvailable()
except OptionalDependencyNotAvailable:
    from ..utils import (
    ...
    _dummy_objects.update(
        get_objects_from_module(dummy_torch_and_transformers_and_opencv_objects)
    )
else:
    _import_structure["consisid"] = ["ConsisIDPipeline"]
